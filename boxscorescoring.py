...
for i in range(0, len(all_game_id)):
    print('Getting boxscore scoring for ' + str(i) + ' ...')
    appended_data.append(pd.DataFrame(boxscorescoringv2.BoxScoreScoringV2(game_id = '00' + str(all_game_id[0][i])).get_data_frames()[0]))
    time.sleep(3)
...
box_score_data.to_csv('2019_20_boxscore_scoring_df.csv', index=False)
